...
\begin{tabular}{\linewidth}{|X|X|X|X|X|}
    ID & Description & Owner & Consequences & Justification of likelihood percentages & Prevention \\
    R-10 & Human error (misuse of software) & Operator & People could get sick & It is very likely that a user makes a mistake as mistakes happen frequently no matter the task. & Popups (i.e. are you sure this is a gluten free item), allows the user to mend their mistake before it becomes an issue. \\
    R-12 & Program freezes while processing customer’s orders & Operator / Team & Angry customers lines get long, lose order & There is a low chance that the system will have a bug that will crash the system once it has been deployed. & Make sure program is stable, allow the user to perform a quick hard reset \\
    R-13 & Screen showing the cooks what orders to make is inconsistent with actual order & Team /2Operator & Angry customers & There is a low chance that the system will have a bug with such an integral part of the system once it has been deployed. & Effective integration testing \\
\end{tabularx}
